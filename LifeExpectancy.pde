//Life expectancy class/tab
class LifeExpectancy extends Visual {
  
  //Column name from CSV file (eg: LifeExpectancy_United States)
  //Using this we can find and loop through each country 
  final String header = "LifeExpectancy_";
  
  //Year Interval for X axis
  final int yearInterval = 10;
  
  //Major tick interval (eg: 30, 40, 50...)
  final float yMajorIntervalTick = 8;
  //Minor tick interval (eg: 35, 45, 55..)
  final float yMinorIntervalTick = 4;
  
  //Holds the min and max value from overall LifeExpectancy columns
  float minYVal = 0.0;
  float maxYVal = 0.0;
      
  //Constructor
  LifeExpectancy(String t, String d, String labelX, String labelY, boolean clicked,  boolean gridFlag, PShape cm,ArrayList<Country> c){
  ...
  LifeExpectancy(String t, String d, String labelX, String labelY, boolean clicked,  boolean gridFlag, Table data, PShape cm, ArrayList<Country> c){
  ...
  //Draw label for XAxis based on the x, y position
  ...
  //Draw label for YAxis based on the x, y position
  ...
  void drawXAxisDataLabel(Table data, float paneX, float paneY, float paneWidth, float paneHeight){
    //Only draw the grid if the flag is true, this flag comes from Visual class
    if(getShowGrid()){
      stroke(224);
      strokeWeight(2);
    }
  ...
  //Draw data label for Y axis (eg: 30 40)
  ...
  void computeMinMaxYVal(Table data){    
    for(int i = 0; i < data.getRowCount(); i++){ //<>//
      for(int j = 0; j < countries.size(); j++){
        //Exclude the country column if its not visible
